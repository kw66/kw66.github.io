  ...
  (function () {
    ...
    function buildLane(side, height, layer) {
      ...
      while (cursor < height + (mascotConfig.size + mascotConfig.gap) * 3) {
        ...
        items.push({ el, side, y: cursor, phaseIndex });
        cursor += denseGap;
        phaseIndex += 1;
      }
      ...
      return {
        ...
        phaseBias,
        headY: -(mascotConfig.size + mascotConfig.gap)
      };
    }
    ...
    function advanceLane(lane, distance) {
      if (lane.direction === 1) {
        ...
        while (lane.headY >= 0) {
          ...
          const newPhase = lane.items[0].phaseIndex - 1;
    ...
    function renderSideMascots() {
      ...
      const rightHeight = Math.max(rightLayer.parentElement ? rightLayer.parentElement.offsetHeight : 0, 960);
    ...
    function spinSlotMachine() {
      ...
      if (!slotMachine || !lever || !tracks.length || slotState.spinning) return;
      ...
      tracks.forEach((track, index) => {
        const sequenceLength = prefersReducedMotion ? 7 + index : 18 + index * 5;
        const sequence = Array.from({ length: sequenceLength }, (_, seqIndex) => {
          return mascotSources[(Math.floor(Math.random() * mascotSources.length) + seqIndex + index) % mascotSources.length];
        });
        ...
        const duration = prefersReducedMotion ? 40 : 720 + index * 320;
        track.style.transition = `transform ${duration}ms cubic-bezier(0.12, 0.92, 0.18, 1)`;
        track.style.transform = `translate3d(0, ${-(sequence.length - 1) * slotItemHeight}px, 0)`;
        ...
        window.setTimeout(handleSettle, duration + 140);
      });
    }
